...
export default function Tracking({ order, type, saleItem }) {
    ...
    return (
        <div className="min-h-screen bg-[#0B0B0B] text-[#F2D9A6] font-sans selection:bg-[#C9A24A] selection:text-black">
            <Head title={__('sale.tracking.head_title')} />
            ...
            <div className="relative max-w-lg md:max-w-xl mx-auto px-4 md:px-6 py-8 md:py-16">
                <Link href={route('sale.index')} className="inline-flex items-center text-[10px] md:text-xs font-black uppercase tracking-widest text-[#C9A24A] hover:text-[#E2C27A] transition mb-6 md:mb-10">
                    <span className="mr-2">&larr;</span> {__('sale.tracking.back')}
                </Link>

                <div className="text-center mb-6 md:mb-8">
                    <h1 className="text-2xl md:text-4xl font-black uppercase tracking-tighter mb-2 md:mb-3">{__('sale.tracking.title_1')} <span className="text-[#C9A24A]">{__('sale.tracking.title_2')}</span></h1>
                    <p className="text-gray-500 text-[10px] md:text-xs max-w-sm mx-auto leading-relaxed px-4 md:px-0">{__('sale.tracking.desc')}</p>
                </div>
                ...
                <form onSubmit={submit} className="mb-8 md:mb-10">
                    <div className="relative group">
                        <input
                            type="text"
                            placeholder={__('sale.tracking.id_ph')}
                            value={data.code}
                            ...
                            required
                        />
                        <label className="absolute left-4 md:left-5 top-2 title-text text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em] text-gray-500 group-focus-within:text-[#C9A24A]">{__('sale.tracking.id_label')}</label>
                        <button
                            disabled={processing}
                        ...
                        >
                            ...
                            <span className="hidden sm:inline ml-1.5 md:ml-2">{__('sale.tracking.button')}</span>
                        </button>
                    </div>
                ...
                {order && order.status === 'Dibatalkan' && (
                    <div className="mb-6 bg-red-500/10 border border-red-500/30 rounded-xl p-4 flex items-center space-x-4 animate-fade-in shadow-lg">
                         ...
                         <div>
                             <h3 className="text-red-500 font-black uppercase text-[10px] tracking-widest mb-0.5">{__('sale.tracking.cancel_title')}</h3>
                             <p className="text-gray-400 text-[10px] leading-tight">{__('sale.tracking.cancel_desc')}</p>
                         </div>
                    </div>
                )}
                ...
                {order && order.status !== 'Dibatalkan' && (
                    <div className="bg-[#151515] rounded-xl md:rounded-2xl p-4 md:p-6 border border-[#222] animate-fade-in shadow-2xl">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 md:mb-8 pb-4 md:pb-5 border-b border-[#222] gap-3 sm:gap-0">
                            <div>
                                <h3 className="text-[10px] md:text-xs font-black uppercase tracking-widest text-gray-500 mb-1 md:mb-2">{__('sale.tracking.curr_status')}</h3>
                                <p className="text-lg md:text-2xl font-black text-[#C9A24A] uppercase tracking-tight">
                                    {order.status === 'Menunggu Konfirmasi' ? __('sale.tracking.status_wait') : 
                                     ...
                                     order.status === 'Selesai' ? __('sale.tracking.status_done') : order.status}
                                </p>
                            </div>
                            <div className="text-left sm:text-right">
                                <h3 className="text-[10px] md:text-xs font-black uppercase tracking-widest text-gray-500 mb-1 md:mb-2">{__('sale.tracking.tracking_id')}</h3>
                        ...
                        <div className="space-y-10 relative">
                            ...
                            <div className="absolute left-5 top-5 bottom-5 w-0.5 bg-[#222]" />
                             ...
                             {[
                                { status: 'Menunggu Konfirmasi', label: __('sale.tracking.status_wait'), sub: __('sale.tracking.status_wait_sub'), icon: ClockIcon },
                                { status: 'Diproses', label: __('sale.tracking.status_process'), sub: __('sale.tracking.status_process_sub'), icon: CheckCircleIcon },
                                { status: 'Dikirim', label: __('sale.tracking.status_ship'), sub: __('sale.tracking.status_ship_sub'), icon: TruckIcon },
                                { status: 'Selesai', label: __('sale.tracking.status_done'), sub: __('sale.tracking.status_done_sub'), icon: CheckCircleIcon }
                            ].map((step, idx) => {
                        ...
                        <div className="mt-8 md:mt-12 pt-6 md:pt-8 border-t border-[#222] grid grid-cols-2 gap-4 md:gap-6">
                            <div>
                                <h4 className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1">{__('sale.tracking.customer_name')}</h4>
                                <p className="text-xs md:text-sm font-bold text-white">{order.customer_name}</p>
                            </div>
                            <div>
                                <h4 className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-500 mb-1">{__('sale.tracking.service_type')}</h4>
                                <p className="text-xs md:text-sm font-bold text-white uppercase tracking-tighter">
                                    {type === 'retail' ? __('sale.tracking.type_retail') : type === 'package' ? __('sale.tracking.type_package') : __('sale.tracking.type_point')}
                                </p>
                            </div>
                        </div>
                        ...
                        {saleItem && (
                            <div className="mt-6 md:mt-8 pt-6 md:pt-8 border-t border-dashed border-[#222]">
                                <h4 className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-500 mb-3 md:mb-4">{__('sale.tracking.ordered_item')}</h4>
